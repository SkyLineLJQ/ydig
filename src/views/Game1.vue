  ...
  <div class="app">
    <main class="container">
        ...
        <span v-for="item in Options" :key="item" class="button-number">
          <template>
            <transition name="bounce">
              <el-button type="warning" :icon="showMore?'iconfont icon-shuzi'+item:'el-icon-star-on'" circle
                @click="handleGetBallValue(item)" v-if="startFlag">
              </el-button>
            </transition>
          </template>
        </span>
      ...
      <div class="roobotsArea">
        <el-card class="box-card user-card">
          <div slot="header" class="clearfix">
            <span>机器人计分板</span><span style="color: #EA0B0B">{{robotClass | formatName}}</span>
            <label style="float: right; padding: 3px 0" type="text">总分:{{sumRobot}}</label>
            <el-popover placement="top-start" title="(´･ω･`)" width="200" trigger="hover" content="~让我偷偷的看看你~">
              <el-link slot="reference" style="float: right;padding: 5px" @click="showResult"><i
          ...
          <div class="text item">
            <el-main style="text-align: center;">
              <span v-for="item in robotValue" :key="item" class="button-number">
                <template>
                  <el-button type="info" :icon="showMore?'iconfont icon-shuzi'+item:'el-icon-star-off'"
                    @click="removeRobot(item)" circle></el-button>
                </template>
              </span>
            </el-main>
            <transition name="slide-fade">
              <el-progress v-if="showMore" type="circle" :percentage="percentRobot" class="progress"></el-progress>
            </transition>
          </div>
        </el-card>
      </div>
